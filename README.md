...
设备上的客户端持续上报原始进程名/包名 + 窗口标题到自托管服务端；朋友打开网页即可实时看到你在干什么。
...
本项目由 DeepSeek-V4-Flash-0731 生成。
...
客户端只上报原始标识（桌面端进程名 / 窗口类名 / bundle id，安卓端包名），**显示名由
服务端渲染**。在**管理面板 → 进程映射**中把 `app_id` 映射成显示名（如 `kitty` → 终端、
`com.tencent.mm` → 微信）；未映射的 `app_id` 原样显示。

- 支持**导入/导出预设**（JSON，形如 `{"kitty":"终端","chrome":"浏览器"}`），可在多台
  服务器间迁移或分享映射表。
- 客户端不再内置映射表。
...
| 平台 | 说明 |
| --- | --- |
| **macOS** | 上报 bundle id（`app_id`）。窗口标题需要「屏幕录制」权限 + CGWindowList API，未实现（见 `clients/desktop/src-tauri/src/foreground/macos.rs`）。 |
| **Linux** | Hyprland（`hyprctl`）、X11 完全支持；wlroots 系 Wayland 视合成器而定；**GNOME/KDE 纯 Wayland 无全局前台 API**，无法读取（会停止上报，保持上次状态）。 |
| **Android** | `app_id` = 包名，`window_title` = 应用名称（读不到"和小明聊天"这类窗口标题——那需要无障碍服务，超出当前范围）。需要「使用情况访问」特殊权限。 |
